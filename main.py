...
class Player(character):
   ...
   def move(self, map) -> None:
      ...
      while True:
         ...
         if (direction.lower() == "n" or direction.lower() == "w") and map.valid_position(self.x, self.y - 1):
            self.y -= 1
            break
         elif (direction.lower() == "e" or direction.lower() == "d") and map.valid_position(self.x + 1, self.y):
            self.x += 1
            break
...
class Map():
   ...
   def print_full_map_w_player(self, player) -> None: #need to edit code for player to be in? also need __str__?
      ...
      for row in self.map:
         ...
         if current_row_index == player.y: #this code inserts the player as "P"
            current_row_str = current_row_str[:player.x] + "P" + current_row_str[player.x + 1:]
   ...
   def print_visible_area(self, player) -> None:
      ...
      for row in self.map:
         ...
         if current_row_index == player.y: #this code inserts the player as "P"
            visible_row_str = visible_row_str[:player.x] + "P" + visible_row_str[player.x + 1:]
